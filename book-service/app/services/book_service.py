...
class BookService:
    ...
    async def update_book(self, book_id: int, book_update: BookInfoUpdate) -> BookResponse:
        book = self.db.query(Book).filter(Book.id == book_id).first()
        if not book:
            raise HTTPException(status_code=404, detail="Book not found")
    ...
    async def update_availability(self, book_id: int, book_update: BookAvailabilityUpdate) -> AvailabilityUpdateResponse:
        book = self.db.query(Book).filter(Book.id == book_id).first()
        if not book:
            raise HTTPException(status_code=404, detail="Book not found")
    ...
    async def search_books(self, search: str) -> list[BookResponse]:
