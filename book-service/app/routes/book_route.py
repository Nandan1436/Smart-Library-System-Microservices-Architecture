...
async def search_books(search: Optional[str] = None, book_service: BookService = Depends(get_book_service)):
    return await book_service.search_books(search)
...
async def update_book(id: int, book: BookInfoUpdate, book_service: BookService = Depends(get_book_service)):
    return await book_service.update_book(id, book)

@router.patch("/{id}/availability", response_model=AvailabilityUpdateResponse)
async def update_book_availability(id: int, book: BookAvailabilityUpdate, book_service: BookService = Depends(get_book_service)):
    return await book_service.update_availability(id, book)
